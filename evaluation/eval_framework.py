...
def generate_synthetic_user_data(
    ...
    rate:List[int] = random.choice([11,7.4,22]),
    total_capacity:List[int] = round(random.uniform(21,123)),
    mean_length_charge:List[int] = round(random.uniform(20000,30000)),
    std_length_charge:List[int] = round(random.uniform(6800,8000)),
    max_percent_capacity:float = 0.95,
    user_charge_tolerance:float = 0.8,
    ) -> pd.DataFrame:


    rate_per_second = np.divide(rate,3600)
    ...
    user_df['total_seconds_to_95'] = user_df['initial_charge'].apply(
        lambda x: total_capacity*(max_percent_capacity-x) / rate * 3600
    )
    ...
    user_df['session_charge'] = user_df[['total_seconds_to_95','length_plugged_in']].min(axis=1)*(rate_per_second)
...
for i in range(1000):
    df_temp = generate_synthetic_user_data(
        distinct_date_list =  distinct_date_list
    )
    dfs.append(df_temp)

all_users = pd.DataFrame(dfs)