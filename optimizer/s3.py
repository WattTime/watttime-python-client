...
class s3_utils():
	...
	def load_file(self,file:str):
		...
		Args:
			file: filename that has to be loaded, i.e. "files/books.csv"
			bucket: bucketname
	...
	def store_pickle_model(self,model_object:'Model Object',file:str, bucket=AWS_S3_BUCKET):
		"""
	...
	def load_csvdataframe(self,file:str,bucket=AWS_S3_BUCKET)->pd.DataFrame:
		"""
		...
		Example:
			data = s3.load_csvdataframe("myfile.csv")
		"""
		data = self.load_file(file=file,bucket=bucket)
		return pd.read_csv(io.BytesIO(data))

	def store_csvdataframe(self,dataframe:pd.DataFrame,key:'filename',bucket=AWS_S3_BUCKET):
		"""
