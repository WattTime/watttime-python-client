...
model.fit(totalCharge=3, totalTime=8, moer=m, charge_per_interval=[3]); model.summary()
print("Two contiguous intervals")
model.fit(totalCharge=3, totalTime=8, moer=m, charge_per_interval=[2,1]); model.summary()
print ("Two contiguous intervals, one of which given as intervals + variable power rate")
model.fit(totalCharge=3, totalTime=8, moer=m, charge_per_interval=[(2,2),1], emission_multiplier_fn=lambda x,y: [1.0,0.1,1.0][x]); model.summary()
print ("Two contiguous intervals, one of which given as intervals + variable power rate")
model.fit(totalCharge=3, totalTime=8, moer=m, charge_per_interval=[2,(1,1)], emission_multiplier_fn=lambda x,y: [1.0,0.1,1.0][x]); model.summary()
print ("Two contiguous intervals, one of which given as intervals + variable power rate")
model.fit(totalCharge=3, totalTime=8, moer=m, charge_per_interval=[(2,2),(1,1)], emission_multiplier_fn=lambda x,y: [1.0,0.1,1.0][x]); model.summary()
