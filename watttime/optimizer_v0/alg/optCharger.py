...
class OptCharger:
    ...
    def __collect_results(self, moer: Moer, emission_multipliers=1.0):
        self.__optimalChargingEmission = moer.get_total_emission(
            np.array(self.__optimalChargingSchedule) * emission_multipliers
        )
    ...
    def __diagonal_fit(
        ...
        emission_multipliers = [
            emission_multiplier_fn(x, y)
            for x, y in zip(optimalPath[:-1, 0], optimalPath[1:, 0])
        ]
    ...
    def fit(
        ...
        if asap:
            ...
            and ra < TOL
            and not constraints
            and emission_multiplier_fn is None
