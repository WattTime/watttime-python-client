...
class TCYCalculator:
    ...
    def _create_reference_table(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        reference = (
            df.groupby(["month", "hour", "is_weekday"])["value"].mean().reset_index()
        )
        return reference

    def _generate_hourly_profile(
        self, year: int, reference: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        profile = profile.merge(
            reference, on=["month", "hour", "is_weekday"], how="left"
        )
